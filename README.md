# AI Meeting Assistant & MoM Generator
...
An intelligent meeting assistant that parses raw multi-speaker transcripts, extracts Minutes of Meeting (MoM), identifies key decisions, and compiles assigned action items.

---

## Key Features

- **Multi-speaker**: transcript parsing and speaker dialogue grouping
- **Automated**: Minutes of Meeting (MoM) synthesis with executive summaries
- **Action**: item extraction with explicit owner assignments and stated deadlines
- **Key**: decisions and consensus tracker
- **One-click**: formatted Markdown summary and team email generator
...
```mermaid
flowchart LR
    Transcript([Raw Meeting Transcript]) --> Parser[Speaker & Dialogue Parser]
    Parser --> Synthesizer[MoM Synthesizer]
    Synthesizer --> Decisions[Decision Registry]
    Synthesizer --> Actions[Assigned Action Items]
    Decisions --> Output[Structured Meeting Brief]
    Actions --> Output
```
...
├── tests/
│   ├── __init__.py
│   └── test_meeting_assistant.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/ai-meeting-assistant.git
...
|---|---|---|
| `POST` | `/process-meeting` | Parse meeting transcript and generate structured MoM and action items |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/process-meeting -H "Content-Type: application/json" -d '{"title": "Product Sync", "transcript": "Alice: We launch on Friday. Bob: I will test staging tomorrow."}'
```
...
```bash
python -m unittest tests/test_meeting_assistant.py
```
